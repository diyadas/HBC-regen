---
title: "wtRegen preparing gene sets"
...
load(file.path(out_dir, paste0(EWT_none_fq_ruv1_nobio_nobatch_all_merge_10PCadjP03.Rda")))

##--remove Fabp5+ cluster from the clusterExperiment object, leaving only the clusters used in the lineage analysis by slingshot
cmobj2M <- cmobjM[,!(primaryClusterNamed(cmobjM) %in% c("-1", "m10"))]
cmobj2O <- cm3[,!(primaryClusterNamed(cm3) %in% c("-1", "c16"))]
save(cmobj2M, cmobj2O, file=file.path(out_dir, paste0(expt_str,"_finalClusterObject_113016.Rda")))
...
save(msigdbSets, bm, file=file.path(out_dir, "MSigDBselectedGeneSets.Rdata"))
