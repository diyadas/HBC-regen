...
nlm_tf <- nlm2[intersect(unlist(read.table("../ref/ATFDB_mm_TF.txt")),rownames(nlm2)),]
slm_tf <- slm2[intersect(unlist(read.table("../ref/ATFDB_mm_TF.txt")), rownames(slm2)),]
```
...
```{r}
de_NL <- read.table(paste0("../output/DE/",expt_str,"/","NL_DE_oneVall_limma_500_2.txt"), stringsAsFactors = FALSE)
de_SL <- read.table(paste0("../output/DE/",expt_str,"/SL_DE_oneVall_limma_500_2.txt"), stringsAsFactors = FALSE)
...
tf_network(nlm_tf, de_NL, col.pal, nlclusters2, "Neur")
tf_network(slm_tf, de_SL, col.pal, slclusters2, "Sus")
```