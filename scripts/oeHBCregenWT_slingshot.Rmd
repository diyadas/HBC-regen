...
library(slingshot); library(clusterExperiment); library(RColorBrewer); library(rgl); library(rglwidget); library(R.utils)
```
```{r setup}
...
cmobj <- cmobj[, !(colnames(cmobjM) %in% exclude_wtlowqc)]
...
table(clus.labels2[colnames(nlm)])
table(clus.labels2[colnames(hlm)])
table(clus.labels2[colnames(h2lm)])
table(clus.labels2[colnames(slm)])
table(clus.labels2)
