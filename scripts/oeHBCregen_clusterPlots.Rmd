...
if (expt_str=="EWT") cluster_ord <- c(9,6,2,5,7,11,12,3,8,4,1)
if (expt_str=="E3") cluster_ord <- c(4,1,12,5,7,2,13,3,6,9,15,16,14)
...
pdf(file = file.path(viz_dir,paste0(expt_str, "_pairsNS_", Sys.Date(), ".pdf")), width=8, height=8)
pairs(t(nlm2)[,c("Omp", "Gap43", "Cnga2", "Sox11", "Cyp2g1", "Cyp1a2")], col=alpha(col.pal[clus.labels[colnames(nlm2)]],1),pch=19, cex=0.5)
legend("bottomright",levels(clus.labels),fill=col.pal,cex=0.5, xpd=TRUE)
dev.off()
...
plot(tsne_data$Y, col=col.pal[clus.labels], pch=19, xaxt='n',yaxt='n',cex=0.5,ylab="",xlab="", main=perplexity)
dev.off()
...
tsne_sub <- function(tsne_data, i){
  plot(tsne_data$Y, col=alpha("grey", 0.5), pch=19, xlab="", ylab='', xaxt='n', yaxt='n', cex=0.5)
  points(tsne_data$Y[expt==levels(expt)[i],], cex=0.5, col=cole[i], pch=19)
}

for (i in 1:sum(grepl("K5ERRY", levels(expt)))){
  tsne_sub(tsne_data,i)
}
dev.off()
```
