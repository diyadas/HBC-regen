...
plotContrasts <- function(DE, contrast_tuple, contrast_name, fname, pval_thresh){
  ...
  tmplist <- apply(contrast_tuple, 1, function(x){
    ...
    # plotting
    png(file.path(DE_dir,paste0(expt_str, "_", contrast_name$names[contrast_name$contrast==desired_contrast],"_volcano_",Sys.Date(),".png")), width=2, height = 2, units="in", res=600)
    par(family='Helvetica', cex.lab=0.5, cex.axis=0.5, oma=c(0,0,0,0), mgp=c(1.2,0.15,0),xpd=F, lwd=0.5, pty="s", mar=c(0.3,0.3,0.3,0.3))
    #layout(c(2,1,1,1))
    with(cont_sub, plot(logFC, -log10(adj.P.Val),xlim=c(-xlim_1,xlim_1), pch=20, col="grey", ylim=c(0,90),cex=0.5, xaxt='n',yaxt='n',xlab='',ylab=''))
    axis(1, lwd = 0.5, tck=-0.01*2, labels=F);axis(2, lwd = 0.5, tck=-0.01*2, labels=F)
    with(subset(cont_sub, adj.P.Val<pval_thresh & abs(logFC)>1), points(logFC, -log10(adj.P.Val), pch=20, col="black", cex=0.5))
    
    #par(mar=c(0.2,1,1,1))
    #with(cont_sub, plot(logFC, -log10(adj.P.Val),xlim=c(-xlim_1,xlim_1), pch=20, col="grey", ylim=c(100,ylim_2),cex=0.5, xaxt='n',yaxt='n',xlab='',ylab=''))
    #axis(2, lwd = 0.5, tck=-0.01*2, labels=TRUE)
    #with(subset(cont_sub, adj.P.Val<pval_thresh & abs(logFC)>1), points(logFC, -log10(adj.P.Val), pch=20, col="black", cex=0.5))
   ...
   #  })
    ...
    png(file.path(DE_dir,paste0(expt_str, "_", contrast_name$names[contrast_name$contrast==desired_contrast],"_tf_volcano_",Sys.Date(),".png")), width=2, height = 2, units="in", res=600)
    par(family='Helvetica', cex.lab=0.5, cex.axis=0.5, oma=c(0,0,0,0), mgp=c(1.2,0.15,0),xpd=F, lwd=0.5, pty="s", mar=c(0.3,0.3,0.3,0.3))
    #layout(c(2,1,1,1))
    with(subset(cont_sub, Feature %in% tfs), plot(logFC, -log10(adj.P.Val),xlim=c(-xlim_1,xlim_1), pch=20, col="grey", ylim=c(0,90),cex=0.5, xaxt='n',yaxt='n',xlab='',ylab=''))
    axis(1, lwd = 0.5, tck=-0.01*2, labels=F);axis(2, lwd = 0.5, tck=-0.01*2, labels=F)
